...
import { readDatabaseUrl } from "@/lib/db-url";
 ...
 * configured sandbox), otherwise a local embedded **PGLite** so the preview
 * always has a working database. Marketplace Postgres may inject POSTGRES_URL
 * instead of DATABASE_URL — both count.
 */
export const dbSource: DbSource = readDatabaseUrl() ? "neon" : "pglite";
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    if (!connectionString) {
      throw new Error("Neon connection string is missing");
    }
